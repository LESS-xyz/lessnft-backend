...
class ScannerAbsolute(threading.Thread):
    ...
    def __init__(self, network: object, type_contract: str, handler: object) -> None:
        super().__init__()
    ...
    def start_polling(self) -> None:
        ...
        while True:
            last_block_checked = get_last_block(self.block_filename, self.network.name)
            ...
            self.handler(
                last_block_checked, last_block_network, self.network, self.type_contract
            ).start()
...
class HandlerDeploy:
    def __init__(
        self, last_block_checked, last_block_network, network, type_contract
    ) -> None:
        self.event = {
    ...
    def start(
        ...
        event_filter = self.event.createFilter(
            fromBlock=self.last_block_checked, toBlock=self.last_block_network
        )
...
class HandlerMintTransferBurn:
    ...
    def save_event(self, event_data):
        ...
        try:
            token = self.get_buyable_token(
                event_args, token_id, collection, smart_contract
            )
        ...
        if old_owner_address == self.empty_address:
            ...
            self.transfer_event(
                token, tx_hash, token_id, new_owner, old_owner, event_args
            )
    ...
    def get_buyable_token(
        self, event_args: dict, token_id: int, collection: Collection, smart_contract
    ) -> Token:
    ...
    def burn_event(
        self, token: Token, tx_hash: str, event_args: dict, old_owner: AdvUser
    ) -> None:
        TokenHistory.objects.get_or_create(
            token=token,
            tx_hash=tx_hash,
    ...
    def change_quantity_old_ownership(
        self, old_owner: AdvUser, token: Token, event_args: dict
    ) -> None:
    ...
    def change_quantity_new_ownership(
        self, new_owner: AdvUser, token: Token, event_args: dict, price
    ) -> None:
        try:
...
class HandlerBuy:
    def __init__(
        self, last_block_checked, last_block_network, network, token_standart
    ) -> None:
        self.event = {
    ...
    def save_event(self, event_data):
        ...
        try:
            token = Token.objects.get(
                collection_address=sell_token, internal_id=token_id
            )
        except Token.DoesNotExist:
            return
        ...
        self.refresh_token_history(event_args, token, tx_hash, new_owner, old_owner)
