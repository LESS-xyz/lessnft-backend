...
class ScannerAbstract(threading.Thread):
    ...
    def start_polling(self) -> None:
        while True:
            ...
            last_block_network = (
                self.network.get_block_data('latest')['number'] - self.network.confirmation_blocks
            )
    ...
    def get_event(self, event_name):
        ...
        f'{self.network.endpoint}/v1/contracts/{self.network.swap_address}/events?event_name={event_name}' \
                f'&min_block_timestamp={last_block_checked["timestamp"]}' \
                f'&max_block_timestamp={last_block_network["timestamp"]}'
        
        event = requests.get(url).json()['data']
        '''
        return event
...
class ScannerDeploy(ScannerAbstract):
    ...
    def save_new_presale(
        ...
        for event in self.events:
            ...
            url = \
            f'{self.network.endpoint}/v1/contracts/{self.network.swap_address}/events?event_name={event}' \
                    f'&min_block_timestamp={last_block_checked["timestamp"]}' \
                    f'&max_block_timestamp={last_block_network["timestamp"]}'
            event_list = requests.get(url).json()['data']
