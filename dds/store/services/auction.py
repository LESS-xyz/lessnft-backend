...
def end_auction(token):
    ...
    tokenToBuy = {
        "tokenAddress": token.collection.network.wrap_in_checksum(
            data.get("tokenToBuy").get("tokenAddress")
        ),
    ...
    tokenToSell = {
        "tokenAddress": token.collection.network.wrap_in_checksum(
            data.get("tokenToSell").get("tokenAddress")
        ),
        "id": int(data.get("tokenToSell").get("id")),
        "amount": int(data.get("tokenToSell").get("amount")),
    }
    feeAddresses = [
        token.collection.network.wrap_in_checksum(
            data.get("fee").get("feeAddresses")[0]
        ),
        token.collection.network.wrap_in_checksum(
            data.get("fee").get("feeAddresses")[1]
        ),
    ]
    feeAmounts = [
